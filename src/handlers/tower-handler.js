...
import { getTower, setTower, removeTower, upgradeTower } from '../models/tower-model.js';
  ...
  try {
    ...
    if (TOWER_TYPE_BUFF !== towerInfo.data.type) {
      if (towerInfo.isGetBuff) {
        ...
        const buffTower = userTowers.find((tower) => tower.positionX === Number(buffTowerPos[0]) && tower.positionY === Number(buffTowerPos[1]));
...
export const attackTowerHandler = (userId, payload) => {
  ...
  try {
    const { towerType, towerId, towerPositionX, towerPositionY, monsterType, monsterId, monsterPositionX, monsterPositionY, timestamp, monsterIndex } = payload;
    ...
    const targetInfo = aliveTargets.find((target) => target.monsterId === monsterId && target.monsterIndex === monsterIndex);
    if (!targetInfo) {
      return {
        status: 'fail',
    ...
    //  4. 공격 사거리 체크
    const distance = Math.sqrt(Math.pow(towerInfo.positionX - calculateX, 2) + Math.pow(towerInfo.positionY - monsterPositionY, 2));
  ...
  try {
    ...
    for (let i = 0; i < userTowers.length; i++) {
      ...
      if ((positionX !== userTowers[i].positionX || positionY !== userTowers[i].positionY) && !userTowers[i].isGetBuff && userTowers[i].data.type !== 'buffer') {
        ...
        if (distance < towerInfo.data.range) {
          changeBuffStatus(userTowers[i], true, positionX + ',' + positionY, towerInfo.data.color);
...
const checkMosterAsset = (type, monsterId) => {
  ...
  if (type === MONSTER_TYPE) {
    // 기준정보 유무 체크 (pawnTowers)
    if (!monsters.data.some((monster) => monster.id === monsterId && monster.type === MONSTER_TYPE)) {
      return 'No monster found for asset';
    }
  } else if (type === BOSS_TYPE) {
    // 기준정보 유무 체크 (pawnTowers)
    if (!monsters.data.some((monster) => monster.id === monsterId && monster.type === BOSS_TYPE)) {
...
const changeBuffStatus = (towerInfo, isBuff, positionXY, color) => {
  ...
  if (color === TOWER_COLOR_BLACK) {
    // 공격
    isBuff ? (towerInfo.data.attack += BUFF_ATTACK_VALUE) : (towerInfo.data.attack -= BUFF_ATTACK_VALUE);
  } else {
    // 공속
    isBuff ? (towerInfo.data.attack_speed += BUFF_ATTACK_SPEED_VALUE) : (towerInfo.data.attack_speed -= BUFF_ATTACK_SPEED_VALUE);
  }
